...
static int32_t last_actual_inc[6] = {0};
static int32_t last_target_inc[6] = {0};
static uint16_t last_status_word[6] = {0};
static uint16_t last_error_code[6] = {0};
static uint16_t last_fault_status_word[6] = {0};
static uint16_t last_fault_error_code[6] = {0};
static double last_actual_deg[6] = {0};
static double last_target_deg[6] = {0};
static std::string last_command = "none";
static std::string last_motion_error = "none";
static std::mutex trajectory_mutex;
static const size_t MAX_WRITE_POINTS = 512;
static const size_t MAX_CARTPATH_POINTS = 512;
static const size_t MAX_TRAJECTORY_QUEUE_VALUES = 300000;
static const double TCP_LINE_MAX_VEL = 50.0;
static const double TCP_LINE_MAX_ACC = 200.0;
static const double TCP_LINE_MAX_DEC = -200.0;
static const double TCP_LINE_MAX_JERK = 50.0;
...
bool axis_operation_enabled(uint16_t status_word) {
	return (status_word & 0x006F) == 0x0027;
}

bool all_axes_operation_enabled() {
	for (int i = 0; i < AXIS_NUM; i++) {
		if (!axis_operation_enabled(last_status_word[i])) return false;
	}
	return true;
}
...
void write_debug_status() {
	...
	out << "flag2=" << flag2 << "\n";
	out << "flags=";
	for (int i = 0; i < SLAVE_NUM; i++) out << flag[i] << (i == SLAVE_NUM - 1 ? "\n" : " ");
	size_t queue_size = 0;
	{
	...
	out << "last_motion_error=" << last_motion_error << "\n";
	out << "actual_inc=";
	for (int i = 0; i < 6; i++) out << last_actual_inc[i] << (i == 5 ? "\n" : " ");
	out << "last_target_inc=";
	for (int i = 0; i < 6; i++) out << last_target_inc[i] << (i == 5 ? "\n" : " ");
	out << "status_word=";
	for (int i = 0; i < 6; i++) out << last_status_word[i] << (i == 5 ? "\n" : " ");
	out << "error_code=";
	for (int i = 0; i < 6; i++) out << last_error_code[i] << (i == 5 ? "\n" : " ");
	out << "last_fault_status_word=";
	for (int i = 0; i < 6; i++) out << last_fault_status_word[i] << (i == 5 ? "\n" : " ");
	out << "last_fault_error_code=";
	for (int i = 0; i < 6; i++) out << last_fault_error_code[i] << (i == 5 ? "\n" : " ");
	out << "fault_axes=";
	bool any_fault = false;
	for (int i = 0; i < 6; i++) {
		if (last_status_word[i] & 0x0008) {
			if (any_fault) out << ",";
			out << "J" << (i + 1);
			any_fault = true;
		}
	}
	if (!any_fault) out << "none";
	out << "\n";
#if ENABLE_GRIPPER_IO
	out << "io_enabled=1\n";
	...
	if (NeedPowerOn || PowerStatus) {
		...
		for (int i = 0; i < 6; i++) current_joints(i) = last_actual_deg[i];
		MatrixXd trans_matrix;
		g_general_6s->calc_forward_kin(current_joints, trans_matrix);
		VectorXd tcp = g_general_6s->tr_2_MCS(trans_matrix);
		out << "actual_tcp=";
		for (int i = 0; i < 6; i++) out << tcp(i) << (i == 5 ? "\n" : " ");
	}
	out << "last_target_deg=";
	for (int i = 0; i < 6; i++) out << last_target_deg[i] << (i == 5 ? "\n" : " ");
	out << "axis_detail: axis actual_deg target_deg actual_inc target_inc inc_error status_word error_code\n";
	for (int i = 0; i < 6; i++) {
		out << "J" << (i + 1) << " "
		    << last_actual_deg[i] << " "
		    << last_target_deg[i] << " "
		    << last_actual_inc[i] << " "
		    << last_target_inc[i] << " "
		    << (last_target_inc[i] - last_actual_inc[i]) << " "
		    << last_status_word[i] << " "
		    << last_error_code[i] << "\n";
	}
}

bool append_line_move(const VectorXd& target, std::deque<double>& trajectory) {
...
bool append_pose_move(const VectorXd& target, std::deque<double>& trajectory) {
	...
	size_t before_size = trajectory.size();
	g_general_6s->move_line_interp(
		target, originMCS, originACS,
		0.0, 0.0,
		0.001, TCP_LINE_MAX_VEL, TCP_LINE_MAX_ACC, TCP_LINE_MAX_DEC, TCP_LINE_MAX_JERK,
		trajectory
	);
	if (trajectory.size() == before_size) {
		printf("[底层] CARTPATH 当前点与目标TCP接近，未生成新的直线插补段。\n");
	}
	return true;
}
...
bool parse_pose_points(std::stringstream& ss, int count, size_t max_count, std::vector<PosePoint>& points, const char* cmd_name) {
	...
	if ((size_t)count > max_count) {
		printf("[底层-拒绝] %s 点数超限: 声明=%d，上限=%zu，未解析，是否截断=否。\n",
	...
	std::string extra;
	if (ss >> extra) {
		printf("[底层-警告] %s 已解析 %d 点，忽略尾部多余字段: %.40s。\n",
			cmd_name, count, extra.c_str());
	}
	return true;
}

void execute_cartesian_path(const std::vector<PosePoint>& points) {
	printf("[底层] CARTPATH 接收点数=%zu，上限=%zu。\n", points.size(), MAX_CARTPATH_POINTS);
	if (points.empty()) {
		printf("[底层-拒绝] CARTPATH 点数为空。\n");
		return;
	}
	if (points.size() > MAX_CARTPATH_POINTS) {
		printf("[底层-拒绝] CARTPATH 点数超限: 接收=%zu，上限=%zu，是否截断=否。\n",
			points.size(), MAX_CARTPATH_POINTS);
		return;
	}
	...
	for (size_t i = 0; i < points.size(); i++) {
		if (!pose_values_finite(points[i])) {
			printf("[底层-拒绝] CARTPATH 第 %zu 点存在非有限数值。\n", i);
			return;
		}
		...
		if (!append_pose_move(target, new_trajectory)) {
			printf("[底层-拒绝] CARTPATH 第 %zu 点逆解或关节跳变检查失败，队列未写入。\n", i);
			return;
		}
		if (new_trajectory.size() > MAX_TRAJECTORY_QUEUE_VALUES) {
			printf("[底层-拒绝] CARTPATH 轨迹队列过长: %zu，上限=%zu，队列未写入。\n",
	...
	printf("[底层] CARTPATH 已接受：接收点数=%zu，实际执行点数=%zu，是否截断=否，队列长度=%zu。\n",
		points.size(), points.size(), new_trajectory.size());
}
...
void execute_write_path(const std::vector<PosePoint>& points, double lift_mm) {
	...
	if (points.size() > MAX_WRITE_POINTS) {
		printf("[底层-拒绝] WRITE 点数超限: 接收=%zu，上限=%zu，是否截断=否。\n",
	...
	for (size_t i = 0; i < points.size(); i++) {
		...
		VectorXd target(6);
		target << points[i].x, points[i].y, points[i].z, points[i].rx, points[i].ry, points[i].rz;
		if (!append_line_move(target, new_trajectory)) {
	...
	printf("[底层] WRITE 已接受：接收点数=%zu，实际执行点数=%zu，是否截断=否，抬笔高度=%.2f mm，队列长度=%zu。\n",
		points.size(), points.size(), lift_mm, new_trajectory.size());
}
...
void cyclic_task() {
	...
	while (1) {
		...
		for (unsigned int i = 0; i < 6; i++) {
			...
			last_actual_inc[i] = actualInc[i];
			last_status_word[i] = EC_READ_U16(domain1_pd + offset.status_word[i]);
			last_error_code[i] = EC_READ_U16(domain1_pd + offset.error_code[i]);
		}
		...
		if (PowerStatus) {
			...
			bool any_axis_not_enabled = false;
			std::stringstream fault_ss;
			std::stringstream not_enabled_ss;
			for (int i = 0; i < 6; i++) {
				if (last_status_word[i] & 0x0008) {
					last_fault_status_word[i] = last_status_word[i];
					last_fault_error_code[i] = last_error_code[i];
					if (any_axis_fault) fault_ss << ",";
					fault_ss << "J" << (i + 1) << "(status=0x" << std::hex
					         << last_status_word[i] << ",error=0x" << last_error_code[i]
					         << std::dec << ")";
					any_axis_fault = true;
				}
				if (!axis_operation_enabled(last_status_word[i])) {
					if (any_axis_not_enabled) not_enabled_ss << ",";
					not_enabled_ss << "J" << (i + 1) << "(status=0x" << std::hex
					               << last_status_word[i] << std::dec << ")";
					any_axis_not_enabled = true;
				}
			}
			if (any_axis_fault) {
				{
				...
				last_motion_error = "轴故障，已清空队列并请求故障复位: " + fault_ss.str();
				printf("[底层-故障] %s\n", last_motion_error.c_str());
				for (int i = 0; i < 6; i++) {
					if (last_status_word[i] & 0x0008) {
						EC_WRITE_U16(domain1_pd + offset.ctrl_word[i], 0x0080);
						EC_WRITE_S32(domain1_pd + offset.target_position[i], actualInc[i]);
						last_target_inc[i] = actualInc[i];
					}
				}
				PowerStatus = 0;
				NeedPowerOn = 1;
				flag2 = 2;
			} else if (any_axis_not_enabled) {
				{
					std::lock_guard<std::mutex> lock(trajectory_mutex);
					std::deque<double> empty_trajectory;
					g_general_6s->set_angle_deque(empty_trajectory);
				}
				last_motion_error = "轴未完全使能，已清空队列并请求重新使能: " + not_enabled_ss.str();
				printf("[底层-警告] %s\n", last_motion_error.c_str());
				for (int i = 0; i < 6; i++) {
					EC_WRITE_S32(domain1_pd + offset.target_position[i], actualInc[i]);
					last_target_inc[i] = actualInc[i];
			...
			std::lock_guard<std::mutex> trajectory_lock(trajectory_mutex);
				if (PowerStatus && !g_general_6s->get_angle_deque().empty()) {
					for (int i = 0; i < 6; i++) {
						...
						int32_t servo_target = g_general_6s->set_target_pos_to_servo(i);
						last_target_inc[i] = servo_target;
						EC_WRITE_S32(domain1_pd + offset.target_position[i], servo_target);
					}
					servo_write_count++;
				} else if (PowerStatus) {
					for (int i = 0; i < 6; i++) {
						EC_WRITE_S32(domain1_pd + offset.target_position[i], last_target_inc[i]);
					}
				}
			}
        ...
        last_command = msg.substr(0, 120);
	        ...
	        if (cmd_type == "MOVEJ") {
	            ...
	            for(int i=0; i<6; i++) {
	                double val;
	                if (!(ss >> val)) {
	                    printf("[底层-拒绝] MOVEJ 指令参数不足，应为: MOVEJ j1 j2 j3 j4 j5 j6。\n");
	                    last_motion_error = "MOVEJ 参数不足";
	                    close(new_socket);
	                    continue;
	                }
	                if (!std::isfinite(val)) {
	                    printf("[底层-拒绝] MOVEJ J%d 目标不是有效数值。\n", i + 1);
	                    last_motion_error = "MOVEJ 目标包含非有限数值";
	                    close(new_socket);
	                    continue;
	                }
	                target(i) = val;
	            ...
	            for (int i = 0; i < 6; i++) {
	                double delta = fabs(target(i) - origin(i));
            ...
            // 写入底层运动队列，EtherCAT 循环会自动消费它
            {
	            ...
	            if (new_trajectory.empty()) {
	                last_motion_error = max_delta < 0.0001
	                    ? "MOVEJ 当前关节角与目标几乎一致"
	                    : "MOVEJ 插补未生成轨迹";
	            }
		            printf("[底层] MOVEJ 插补完成，轨迹队列长度=%zu，PowerStatus=%d，最大关节偏差=%.4f deg，J5 当前=%.4f deg，目标=%.4f deg。\n",
		                   new_trajectory.size(), PowerStatus, max_delta, origin(4), target(4));
		        }
	        else if (cmd_type == "RESETJ5") {
	            {
	                std::lock_guard<std::mutex> lock(trajectory_mutex);
	                std::deque<double> empty_trajectory;
	                g_general_6s->set_angle_deque(empty_trajectory);
	            }
	            EC_WRITE_U16(domain1_pd + offset.ctrl_word[4], 0x0080);
	            EC_WRITE_S32(domain1_pd + offset.target_position[4], last_actual_inc[4]);
	            last_target_inc[4] = last_actual_inc[4];
	            last_target_deg[4] = last_actual_deg[4];
	            PowerStatus = 0;
	            NeedPowerOn = 1;
	            flag2 = 2;
	            last_motion_error = "RESETJ5 已发送故障复位，等待重新使能";
	            printf("[底层] RESETJ5: status=0x%04x error=0x%04x actual=%.4f deg inc=%d。\n",
	                   last_status_word[4], last_error_code[4], last_actual_deg[4], last_actual_inc[4]);
	        }
	        else if (cmd_type == "HOMEJ5") {
	            if (last_status_word[4] & 0x0008) {
	                last_motion_error = "HOMEJ5 拒绝：J5 仍处于故障状态，请先 RESETJ5";
	                printf("[底层-拒绝] HOMEJ5: J5 still fault, status=0x%04x error=0x%04x。\n",
	                       last_status_word[4], last_error_code[4]);
	                close(new_socket);
	                continue;
	            }
		            if (!PowerStatus) {
		                last_motion_error = "HOMEJ5 拒绝：伺服未重新使能";
		                printf("[底层-拒绝] HOMEJ5: PowerStatus=0，请等待重新使能。\n");
		                close(new_socket);
		                continue;
		            }
		            if (!all_axes_operation_enabled()) {
		                last_motion_error = "HOMEJ5 拒绝：6轴未全部 Operation Enabled";
		                printf("[底层-拒绝] HOMEJ5: axes are not all operation enabled。\n");
		                close(new_socket);
		                continue;
		            }
	            VectorXd origin(6);
	            ...
	            g_general_6s->move_joint_interp(
	                target, origin, vel_current, acc_current,
	                0.001, 5, 10, 10, 10, new_trajectory
	            );
	            {
	                std::lock_guard<std::mutex> lock(trajectory_mutex);
	                g_general_6s->set_angle_deque(new_trajectory);
	            ...
	            if (new_trajectory.empty()) {
	                last_motion_error = "HOMEJ5 未生成轨迹";
	            } else {
	                last_motion_error = "none";
	            }
		            printf("[底层] HOMEJ5: J5 %.4f -> 0.0000 deg，队列长度=%zu。\n",
		                   origin(4), new_trajectory.size());
		        }
	        else if (cmd_type == "STEPJ5") {
	            double step_deg = 2.0;
	            ss >> step_deg;
	            if (!std::isfinite(step_deg) || step_deg <= 0.0 || step_deg > 5.0) {
	                step_deg = 2.0;
	            }
	            if (last_status_word[4] & 0x0008) {
	                last_motion_error = "STEPJ5 拒绝：J5 仍处于故障状态，请先 RESETJ5";
	                printf("[底层-拒绝] STEPJ5: J5 still fault, status=0x%04x error=0x%04x。\n",
	                       last_status_word[4], last_error_code[4]);
	                close(new_socket);
	                continue;
	            }
		            if (!PowerStatus) {
		                last_motion_error = "STEPJ5 拒绝：伺服未重新使能";
		                printf("[底层-拒绝] STEPJ5: PowerStatus=0，请等待重新使能。\n");
		                close(new_socket);
		                continue;
		            }
		            if (!all_axes_operation_enabled()) {
		                last_motion_error = "STEPJ5 拒绝：6轴未全部 Operation Enabled";
		                printf("[底层-拒绝] STEPJ5: axes are not all operation enabled。\n");
		                close(new_socket);
		                continue;
		            }
	            VectorXd origin(6);
	            ...
	            if (fabs(origin(4)) <= step_deg) {
	                target(4) = 0.0;
	            } else {
	                target(4) = origin(4) + (origin(4) > 0.0 ? -step_deg : step_deg);
	            }
	            ...
	            if (new_trajectory.empty()) {
	                last_motion_error = "STEPJ5 未生成轨迹";
	            } else {
	                last_motion_error = "none";
	            }
	            printf("[底层] STEPJ5: J5 %.4f -> %.4f deg，队列长度=%zu。\n",
	                   origin(4), target(4), new_trajectory.size());
	        }
	        else if (cmd_type == "MOVEL") {
            last_motion_error = "none";
            VectorXd target(6);
            PosePoint targetPoint;
            bool parse_ok = true;
            for(int i=0; i<6; i++) {
                double val;
                if (!(ss >> val)) {
                    printf("[底层-拒绝] MOVEL 指令参数不足，应为: MOVEL x y z rx ry rz。\n");
                    last_motion_error = "MOVEL 参数不足";
                    parse_ok = false;
                    break;
                }
                target(i) = val;
            ...
            targetPoint.x = target(0);
            targetPoint.y = target(1);
            targetPoint.z = target(2);
            targetPoint.rx = target(3);
            targetPoint.ry = target(4);
            targetPoint.rz = target(5);

            if (!pose_values_finite(targetPoint)) {
                printf("[底层-拒绝] MOVEL 目标包含非有限数值。\n");
                last_motion_error = "MOVEL 目标包含非有限数值";
                close(new_socket);
                continue;
            }
            if (!pose_in_safe_range(targetPoint)) {
                printf("[底层-拒绝] MOVEL 目标超出安全范围: %.3f %.3f %.3f。\n",
                    targetPoint.x, targetPoint.y, targetPoint.z);
                last_motion_error = "MOVEL 目标超出安全范围";
                close(new_socket);
                continue;
            }
            ...
            bool joint_jump_ok = true;
            for (int i = 0; i < 6; i++) {
                if (fabs(targetACS(i) - originACS(i)) > 45.0) {
                    printf("[底层-拒绝] MOVEL 单段关节变化过大: J%d %.3f -> %.3f。\n",
                        i + 1, originACS(i), targetACS(i));
                    std::stringstream err_ss;
                    err_ss << "MOVEL 单段关节变化过大 J" << (i + 1) << " "
                           << originACS(i) << " -> " << targetACS(i);
                    last_motion_error = err_ss.str();
                    joint_jump_ok = false;
                    break;
                }
            }
            ...
            if (new_trajectory.size() > MAX_TRAJECTORY_QUEUE_VALUES) {
                printf("[底层-拒绝] MOVEL 轨迹队列过长: %zu，上限=%zu，队列未写入。\n",
                    new_trajectory.size(), MAX_TRAJECTORY_QUEUE_VALUES);
                last_motion_error = "MOVEL 轨迹队列过长";
                close(new_socket);
                continue;
            }
            if (new_trajectory.empty()) {
                ...
                for (int i = 0; i < 6; i++) {
                    max_joint_delta = std::max(max_joint_delta, fabs(targetACS(i) - originACS(i)));
                }
                if (max_joint_delta > 0.0001) {
                    VectorXd vel_current = VectorXd::Zero(6);
                    VectorXd acc_current = VectorXd::Zero(6);
                    g_general_6s->move_joint_interp(
                        targetACS, originACS, vel_current, acc_current,
                    ...
                    printf("[底层] MOVEL 位置变化很小，改用慢速姿态插补，最大关节变化=%.6f，队列长度=%zu。\n",
                        max_joint_delta, new_trajectory.size());
                    last_motion_error = new_trajectory.empty()
                        ? "MOVEL 姿态兜底插补仍为空"
                        : "MOVEL 位置变化很小，已用慢速姿态兜底插补";
                } else {
                    printf("[底层] MOVEL 当前TCP与目标几乎一致，未生成新的插补段。\n");
                    last_motion_error = "MOVEL 当前TCP与目标几乎一致";
                }
            }
            ...
            printf("[底层] MOVEL 直线插补完成，当前TCP=(%.3f %.3f %.3f %.3f %.3f %.3f)，目标TCP=(%.3f %.3f %.3f %.3f %.3f %.3f)，轨迹队列长度=%zu，PowerStatus=%d。\n",
                originMCS(0), originMCS(1), originMCS(2), originMCS(3), originMCS(4), originMCS(5),
            ...
            int count = 0;
            if (!(ss >> count)) {
                printf("[底层-拒绝] CARTPATH 指令头解析失败，应为: CARTPATH count x y z rx ry rz ...。\n");
                close(new_socket);
                continue;
            }

            std::vector<PosePoint> points;
            if (parse_pose_points(ss, count, MAX_CARTPATH_POINTS, points, "CARTPATH")) {
