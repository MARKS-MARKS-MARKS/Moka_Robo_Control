...
static const bool INVERT_IK_AXIS5_DELTA = true;
...
void General_6S::calc_inverse_kin(const MatrixXd& transMatrix,const VectorXd& posLast, VectorXd& posACS)
{
	...
	if (INVERT_IK_AXIS5_DELTA) {
		posACS[4] = posLast[4] - (posACS[4] - posLast[4]);
	}
}
		...
		double maxJerk, std::deque<double> &deque)
{
	...
	if (displacement > 0.2)
	{
        ...
        // Use Cartesian timing for MOVEL. The ACS timing above can make TCP moves
        // unexpectedly fast because it is based on joint rated velocity.
        rlst = rlstMcs;
	}
	else
	{
